"""IDP Temporal worker — registers IDP workflow + activities.

Entry point: python -m agent_harness.workers.idp
"""
...
def get_workflow_list() -> list[type]:
    """Return workflow classes for the IDP worker."""
    return [IdpOperativoWorkflow]


def get_activity_list() -> list:
    """Return activity functions for the IDP worker.
...
async def run_worker() -> None:
    """Start the IDP worker.

    Connects to Temporal, creates a Worker bound to the IDP task queue,
    registers the IDP workflow and all activity implementations, and runs
...
def main() -> None:
    """Entry point for python -m agent_harness.workers.idp."""
