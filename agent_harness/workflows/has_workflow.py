...
class CEEWorkflowConfig:
    """Configuration for the HAS workflow."""
...
@workflow.defn
class CEEOperativoWorkflow:
    """HAS operativo workflow — Phases 0-6.

    Phase 0: Intake (gateway submits workflow)
    Phase 1: Santos writes plan
    Phase 2: Medina investigates (injection scan + field extraction)
    ...
    def __init__(self, config: CEEWorkflowConfig | None = None) -> None:
        self.config = config or CEEWorkflowConfig()

    def build_plan_input(self, operativo_id: str, input: CEEOperativoInput) -> PlannerInput:
        """Phase 1: Build input for Santos planning activity."""
        return PlannerInput(
            operativo_id=operativo_id,
            domain="has",
            pdf_description=f"HAS document: {input.document_filename} (type={input.document_type})",
        )

    def build_investigate_input(
        self, operativo_id: str, input: CEEOperativoInput
    ) -> InvestigatorInput:
        ...
        return InvestigatorInput(
            ...
            pdf_filename=input.document_filename,
        )
    ...
    @workflow.run
    async def run(self, input: CEEOperativoInput) -> CEEOperativoOutput:
        """Execute the full HAS operativo workflow (Phases 1-6)."""
        ...
        investigate_output: InvestigatorOutput = await workflow.execute_activity(
            "medina_investigate",
            investigate_input,
            start_to_close_timeout=timedelta(seconds=self.config.investigate_timeout_seconds),
        )
        progress_entries.append(f"Phase 2 (Investigate): {investigate_output.phase_result}")
        ...
        # Phase 3: Lamponne executes
        execute_input = self.build_execute_input(operativo_id, plan_output.plan_json)
        execute_output: AgentLoopOutput = await workflow.execute_activity(
            "lamponne_execute",
            execute_input,
            start_to_close_timeout=timedelta(seconds=self.config.execute_timeout_seconds),
        )
        ...
        synthesize_output: SynthesizerOutput = await workflow.execute_activity(
            "ravenna_synthesize",
            synthesize_input,
            start_to_close_timeout=timedelta(seconds=self.config.synthesize_timeout_seconds),
        )
        ...
        await workflow.execute_activity(
            "post_job_learn",
            post_job_input,
            start_to_close_timeout=timedelta(seconds=self.config.post_job_timeout_seconds),
        )
    ...
    async def _qa_with_retry(
        ...
        for attempt in range(self.config.max_correction_attempts):
            ...
            qa_output: QAReviewOutput = await workflow.execute_activity(
                "santos_qa_review",
                qa_input,
                start_to_close_timeout=timedelta(seconds=self.config.qa_timeout_seconds),
            )
...
CEEWorkflow = CEEOperativoWorkflow
